...
func main() {
	...
	if err := os.Chdir(executableDir); err != nil {
		...
		logrus.WithError(err).Fatalf("Chdir to %v failed", executableDir)
	}

	curDir, err := os.Getwd()
	if err != nil {
		logrus.WithError(err).Fatalf("cannot get current working dir")
	}
	logrus.Infof("Current working dir:%s", curDir)
